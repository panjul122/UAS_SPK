...
def itemData():
	...
	for row in f:
		...
		g = r[len(r)-1].split('|')
		for e in g:
			if e.ip() not in genre.keys():
				continue
			else:
				data[int(r[0])-1][genre[e.ip()]] = 1
...
def similarity_item(data):
	...
	for item1 in range(items):
		...
		for item2 in range(items):
			...
			#f_i_d.write((item1) + "," + (item2) + "," + (item_similarity_cosine[item1][item2]) + "," + (item_similarity_jaccard[item1][item2]) + "," + (item_similarity_pearson[item1][item2]) + "\n")
	#f_i_d.close()
...
def similarity_user(data):
	...
	for user1 in range(users):
		...
		for user2 in range(users):
			...
			#f_i_d.write((user1) + "," + (user2) + "," + (user_similarity_cosine[user1][user2]) + "," + (user_similarity_jaccard[user1][user2]) + "," + (user_similarity_pearson[user1][user2]) + "\n")
	#f_i_d.close()
...
def crossValidation(data, user_data, item_data):
	...
	for train_indices, test_indices in k_fold:
		...
		for e in test:
			...
			#pred_jaccard = (user_pred_jaccard + item_pred_jaccard)/2
			#pred_pearson = (user_pred_pearson + item_pred_pearson)/2
			print (user) + "\t" + (item) + "\t" + (e[2]) + "\t" + (pred_cosine) + "\t" + (pred_jaccard) + "\t" + (pred_pearson)
		...
		print (sqrt(mean_squared_error(true_rate, pred_rate_cosine))) + "\t" + (sqrt(mean_squared_error(true_rate, pred_rate_jaccard))) + "\t" + (sqrt(mean_squared_error(true_rate, pred_rate_pearson)))
		#raw_input()
	...
	print (rmse_cosine) + "\t" + (rmse_jaccard) + "\t" + (rmse_pearson)

	f_rmse = open("rmse_hybrid.txt","w")
	f_rmse.write((rmse_cosine) + "\t" + (rmse_jaccard) + "\t" + (rmse_pearson) + "\n")

	rmse = [rmse_cosine, rmse_jaccard, rmse_pearson]
	req_sim = rmse.index(min(rmse))

	print (req_sim)
	f_rmse.write((req_sim))
	f_rmse.close()
...
def predictRating(data, user_data, item_data):
	...
	for e in range(l):
		...
		print (user) + "," + (item) + "," + (pred)
		#fw.write((user) + "," + (item) + "," + (pred) + "\n")
		fw_w.write((pred) + "\n")
